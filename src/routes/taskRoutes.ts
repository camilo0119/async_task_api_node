...
/**
 * Inicializa las rutas de tareas con todas las dependencias cargadas.
 */
export async function initTaskRoutes(): Promise<Router> {
  const router = Router();

  const redis = await getRedisConnection();
  const queueService = new QueueService(redis);
  const taskController = new TaskController(queueService);
